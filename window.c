...
void win_init(win_t *win)
{
	...
	bfg = win_res(db, RES_CLASS ".bar.foreground", bg);
	win_alloc_color(e, bg, &win->bg);
	win_alloc_color(e, fg, &win->fg);
	win_alloc_color(e, bbg, &win->bbg);
	win_alloc_color(e, bfg, &win->bfg);
...
void win_draw_bar(win_t *win)
{
	...
	XSetForeground(e->dpy, gc, win->bfg.pixel);
	XSetBackground(e->dpy, gc, win->bbg.pixel);
	...
	if ((len = strlen(r->buf)) > 0) {
		...
		w -= tw;
		win_draw_text(win, d, &win->bfg, x, y, r->buf, len, tw);
	}
	if ((len = strlen(l->buf)) > 0) {
		x = H_TEXT_PAD;
		w -= 2 * H_TEXT_PAD; /* gap between left and right parts */
		win_draw_text(win, d, &win->bfg, x, y, l->buf, len, w);
	}
	XftDrawDestroy(d);
}
